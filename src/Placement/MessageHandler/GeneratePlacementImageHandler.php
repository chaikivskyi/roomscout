...
namespace App\Placement\MessageHandler;
...
final class GeneratePlacementImageHandler
{
    ...
    public function __invoke(GeneratePlacementImageMessage $message): void
    {
        $placement = $this->entityManager->find(ProductPlacement::class, $this->parsePlacementId($message->placementId));

        if (null === $placement) {
            $this->logger->info('Skipping generation: placement was deleted.', ['placementId' => $message->placementId]);
