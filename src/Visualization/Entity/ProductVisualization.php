...
namespace App\Visualization\Entity;
...
#[ORM\Entity(repositoryClass: ProductVisualizationRepository::class)]
#[ORM\HasLifecycleCallbacks]
#[ORM\UniqueConstraint(
    name: 'uniq_active_visualization_per_project',
    columns: ['project_id'],
    options: ['where' => "((status)::text = 'processing'::text)"],
)]
class ProductVisualization implements FailableInterface
{
    #[ORM\Id]
    ...
    #[ORM\Column(length: 16, enumType: VisualizationStatus::class)]
    ...
    public function __construct(
        ...
        $this->status = VisualizationStatus::Processing;
    ...
    public function getStatus(): VisualizationStatus
    {
        return $this->status;
    }
    ...
    {
        if (VisualizationStatus::Completed === $this->status) {
            return false;
        }

        $this->status = VisualizationStatus::Completed;
    ...
    {
        if (VisualizationStatus::Processing !== $this->status) {
            return false;
        }

        $this->status = VisualizationStatus::Failed;
